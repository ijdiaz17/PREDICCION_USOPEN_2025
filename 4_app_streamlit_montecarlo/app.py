...
# Se importa la función de carga principal y los datos fijos del cuadro.
from main import load_data_and_caches, cuadro_usopen2025 
from utils import simular_torneo_run_aleatorio 
...
# Asumo que esta línea es la que usa la variable desempaquetada en main.py:
...
st.set_page_config(
    page_title="Simulación US Open 2025",
    layout="wide"
)
...
st.title("🎾 Predicción del US Open 2025")
st.markdown(
    """
    ### ¿Quién será el campeón del último Grand Slam del año?
    ---
    """, unsafe_allow_html=True 
)
...
with col_b:
    if st.button("🔥 PREDECIR CAMPEÓN 🔥", type="primary"):
        
        with st.spinner("Ejecutando Simulación Monte Carlo..."):
            # 1. Ejecutar el Monte Carlo (rápido gracias al caching H2H)
            df_progress = simular_torneo_run_aleatorio(
        ...
        st.subheader("📊 Resultados Proyectados (desde octavos de final)")
        ...
        st.info("Clickeá para obtener la predicción a través de la técnica Monte Carlo.")